...
let identifier = match &parser.current_token.take() {
    ...
    _ => Err(("expected identifier name following let | const keywords".to_owned(), parser.lexer.span())),
};
...
match &parser.current_token {
    Some(Ok(TokenType::Semicolon)) => {
        if(is_const){
         return Err(("Must assign value to const declaration.".to_owned(), parser.lexer.span()))
        }         
    ...
    Some(Ok(TokenType::Equals)) => {
        ...
        match &parser.current_token {
            ...
            _ => Err(("expected semicolon after variable declaration".to_owned(), parser.lexer.span())),
        }
    },
    _ => Err(("expected equals sign after identifier name".to_owned(), parser.lexer.span())),
}
